...
# ── Live Global Indices (Price + 1D%, yfinance, refreshed every 30s) ──
# Index display name -> yfinance symbol. Mirrors database.py's
# GlobalIndicesEngine maps; only entries with a real yfinance symbol are
# kept (delisted/None ones are dropped). Names are matched stripped of
# surrounding whitespace against the sheet's index-name column.
GLOBAL_INDEX_TICKERS = {
    # Table 1 — major indices
    "KOSPI": "^KS11", "CAC 40 INDEX": "^FCHI", "CAC 40": "^FCHI",
    "FTSE 100": "^FTSE", "DAX INDEX": "^GDAXI", "DAX": "^GDAXI",
    "Nasdaq Composite": "^IXIC", "Hang Seng": "^HSI", "S&P 500": "^GSPC",
    "ASX 200 INDEX": "^AXJO", "ASX 200": "^AXJO", "Dow Jones (DJIA)": "^DJI",
    "Dow Jones": "^DJI", "NIFTY 50": "^NSEI", "SENSEX": "^BSESN",
    "Nikkei 225": "^N225", "Shanghai Composite": "000001.SS",
    # Table 2 — additional indices
    "TAIEX": "^TWII", "Jakarta Composite": "^JKSE", "PSEi": "PSI20.LS",
    "SET Index": "^SET.BK", "XU100 (BIST 100)": "XU100.IS", "TA-35": "TA35.TA",
    "TA-125": "^TA125.TA", "ASX 50": "^AFLI", "BEL 20": "^BFX", "SMI": "^SSMI",
    "All Ordinaries": "^AORD", "TSX Composite": "^GSPTSE", "IBOV (IBOVESPA)": "^BVSP",
    "MEXBOL (IPC)": "^MXX", "NYSE Composite": "^NYA", "NIFTY Bank": "^NSEBANK",
    "NIFTY Next 50": "^NSMIDCP", "NASDAQ 100": "^NDX", "ATX": "^ATX",
    "NZX 50": "^NZ50", "WIG20": "WIG20.WA", "IBEX 35": "^IBEX",
    "FTSEMIB": "FTSEMIB.MI", "FTSE MIB": "FTSEMIB.MI", "OBX": "OSEBX.OL",
    "TASI": "^TASI.SR", "STOXX Europe 600": "^STOXX", "AMX": "^AMX",
    "SZSE Component": "399001.SZ", "S&P MidCap 400": "^SP400",
    "SX5E (Eurozone)": "^STOXX50E", "SX5E / EURO STOXX 50": "^STOXX50E",
    "EURO STOXX 50": "^STOXX50E", "CSI 300": "000300.SS", "Russell 2000": "^RUT",
    "SDAX": "^SDAXI", "Wilshire 5000": "^W5000", "MDAX": "^MDAXI",
    "NIFTY FMCG": "^CNXFMCG", "Straits Times": "^STI", "OMX Stockholm 30": "^OMX",
    "AEX": "^AEX", "FTSE 350": "^FTLC", "TecDAX": "^TECDAX", "CAC Next 20": "^CN20",
    "OMX Helsinki 25": "^OMXH25", "SA40 (JSE Top 40)": "^J200.JO",
    "SA40 / JSE Top 40": "^J200.JO", "KLCI": "^KLSE", "Hang Seng Tech": "3033.HK",
    "Hang Seng China Enterprises": "^HSCE", "mWIG40": "mWIG40.WA",
    "OMX Copenhagen 25": "^OMXC25", "ISEQ 20": "^ISEQ", "FTSE All World": "^VXUS",
    "Dow Jones Global Titans 50": "^DJGT",
}


@st.cache_data(ttl=30)
def fetch_global_live(symbols: tuple) -> dict:
    """Batch-fetch live Price + 1D% for the given yfinance symbols.
    Returns {yf_symbol: {"price": float, "chg1d": float}}. Cached 30s so
    concurrent viewers share one Yahoo call. Returns {} on any failure so
    callers fall back to the sheet values."""
    if not symbols:
        return {}
    try:
    ...
    for sym in symbols:
        try:
            ...
            chg1d = (price / prev - 1) * 100 if prev else None
...
def apply_global_live(df: pd.DataFrame, name_col: str) -> pd.DataFrame:
    """Overwrite the Price and 1D columns of a Global Indices DataFrame with
    live yfinance values, matched by (stripped) index name. Rows without a
    live match keep their sheet values. On fetch failure the df is unchanged."""
    if df.empty or name_col not in df.columns:
        return df
    # name -> symbol for the rows present in this table
    name_to_sym = {}
    for nm in df[name_col]:
        sym = GLOBAL_INDEX_TICKERS.get(str(nm).strip())
        if sym:
            name_to_sym[nm] = sym
    if not name_to_sym:
        return df
    live = fetch_global_live(tuple(sorted(set(name_to_sym.values()))))
    if not live:
        return df
    df = df.copy()
    has_price = "Price" in df.columns
    has_1d = "1D" in df.columns
    for i, nm in df[name_col].items():
        sym = name_to_sym.get(nm)
        q = live.get(sym) if sym else None
        if not q:
            continue
        if has_price and q.get("price") is not None:
            df.at[i, "Price"] = f"{q['price']}"
        if has_1d and q.get("chg1d") is not None:
            df.at[i, "1D"] = f"{q['chg1d']}"
    return df
